...
// Swift Testing's @Test macro is rejected inside an extension on a
// specialized generic type (`TextFragment<AttributedString>`), so the
// suite lives at the top level and references the specialization
